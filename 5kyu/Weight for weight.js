...
function orderWeight(string) {
    return string
      ...
      .sort((a, b) => {
         ...
         else return newOrder(a) - newOrder(b);
         })
      .join(' '); 
 }
 
 function newOrder(number) {
   return number.split('').reduce((acc, item) => acc + +item, 0)
 }
...
function orderWeight(strng) {                                           //exact same as my solution. Just harder to read
