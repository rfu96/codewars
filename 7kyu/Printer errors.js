...
function printerError(s) {
  ...
  let sArray = s.split('')
  totalChar = sArray.length;
  
  sArray.forEach((item) => {
    if (!regex.test(item)) error += 1;
  });
  
  return `${error}/${totalChar}`
}
...
function printerError(s) {
    ...
    for(var i = 0; i < s.length; i++) {
      if (s[i] > "m") {                             //didn't know you could '>' a character wow
        count++;
      }
    }
...
const printerError = s => `${s.replace(/[a-m]/gi, "").length}/${s.length}`;   //this is a much better implementation of what i was going for
